...
//
// Researcher browser dashboard served by the Main Hub at /uat.
...
// on the participant dashboard. It also provides exactly one controlled
// UAT stimulus: the validated one-strong-vote WARNING path. Fusion remains
// authoritative and there is intentionally no web EMERGENCY injection.
// ============================================================
...
<div class="wrap">
  ...
  <section class="card" style="margin-bottom:12px">
    <h2>Controlled UAT Stimulus <span class="badge">RESEARCHER ONLY</span></h2>
    <div class="stimBox">
      <div class="k">Stimulus state</div>
      <div class="stimState" id="stim-state">CHECKING...</div>
      <div class="stimNote" id="stim-note">This control does not fake the participant app status. It injects one validated strong FSR model vote before Fusion; Fusion remains authoritative. No Emergency web injection is available.</div>
    </div>
    <div class="actions">
      <button class="btnWarn" id="stim-warn-btn" onclick="simulateWarning()">Simulate Warning</button>
      <button class="btnClear" id="stim-clear-btn" onclick="clearSimulation()">Clear Simulation</button>
    </div>
    <div class="rec">Use one controlled Warning per participant. Clear it after the participant has observed/interpreted the Warning. CSV recordings include the stimulus state for transparent reporting.</div>
  </section>
...
let uatStimulus='OFF',stimulusBusy=false;
...
function postJson(path,timeoutMs=4000){return new Promise((resolve,reject)=>{const x=new XMLHttpRequest();x.open('POST',HUB_BASE+path,true);x.timeout=timeoutMs;x.setRequestHeader('Accept','application/json');x.onreadystatechange=()=>{if(x.readyState!==4)return;let body=null;try{body=JSON.parse(x.responseText||'{}')}catch(e){}if(x.status>=200&&x.status<300){resolve(body||{})}else{reject(new Error((body&&body.hint)||((body&&body.error)||('HTTP '+x.status))))}};x.onerror=()=>reject(new Error('Network error requesting '+path));x.ontimeout=()=>reject(new Error('Timeout requesting '+path));x.send('')})}
function renderStimulus(mode,note){uatStimulus=mode||'OFF';const active=uatStimulus==='CONTROLLED_WARNING';const el=$('stim-state');el.textContent=active?'CONTROLLED WARNING ACTIVE':uatStimulus;el.className='stimState '+(active?'warn':'ok');if(note)$('stim-note').textContent=note;$('stim-warn-btn').disabled=stimulusBusy||active;$('stim-clear-btn').disabled=stimulusBusy||uatStimulus==='OFF'}
async function refreshStimulus(){try{const d=await requestJson('/api/v1/uat/stimulus',2500);renderStimulus(d.mode||'OFF')}catch(e){$('stim-state').textContent='STATUS UNAVAILABLE';$('stim-state').className='stimState bad'}}
async function simulateWarning(){if(stimulusBusy)return;if(!confirm('Start the controlled UAT Warning stimulus?\n\nUse this only while the participant is seated and monitoring is active.'))return;stimulusBusy=true;$('stim-warn-btn').disabled=true;$('stim-clear-btn').disabled=true;try{const d=await postJson('/api/v1/uat/simulate-warning');renderStimulus(d.mode||'CONTROLLED_WARNING','Controlled Warning enabled. Wait for the participant app to change through the real Fusion/API path.')}catch(e){alert('Could not start controlled Warning: '+e.message);await refreshStimulus()}finally{stimulusBusy=false;renderStimulus(uatStimulus)}}
async function clearSimulation(){if(stimulusBusy)return;stimulusBusy=true;$('stim-warn-btn').disabled=true;$('stim-clear-btn').disabled=true;try{const d=await postJson('/api/v1/uat/clear-simulation');renderStimulus(d.mode||'OFF','Stimulus cleared. Fusion will return toward SAFE according to normal production hysteresis.')}catch(e){alert('Could not clear simulation: '+e.message);await refreshStimulus()}finally{stimulusBusy=false;renderStimulus(uatStimulus)}}
...
window.addEventListener('load',()=>{refreshStimulus();pollTimer=setTimeout(tick,750)});
</script>
</body>
</html>
)rawliteral";
